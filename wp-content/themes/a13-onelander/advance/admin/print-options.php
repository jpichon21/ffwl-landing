...
function apollo13framework_print_form_controls( $option, &$params, $is_meta = false ) {
	...
	if ( $option['type'] == 'upload' ) {
		$upload_button_text = ! empty( $option['button_text'] ) ? $option['button_text'] : esc_html__( 'Upload', 'a13-onelander' );
		?>
		...
		<div class="upload-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				?> />
				<input id="clear_<?php echo esc_attr($input_prefix . $option['id']); ?>" class="clear-image-button button" type="button" value="<?php echo esc_attr__( 'Clear field', 'a13-onelander' ) ?>" />
	...
	elseif ( $option['type'] == 'color' ) {
		...
		<div class="color-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				<?php apollo13framework_input_help_tip( '<p>'.__( 'Use the correct <code>color</code> property in CSS( <code>green, #33FF99, rgb(255,128,0), rgba(222,112,12,0.5)</code> ), or choose a color with a color picker. Leave an empty value to use the default theme value.', 'a13-onelander' ).'</p><p>'.__( 'Use the "Transparent" button to insert a transparent value.', 'a13-onelander' ) ); ?>
				...
				<button class="transparent-value button-secondary"><?php esc_html_e( 'Transparent', 'a13-onelander' ); ?></button>
				<p class="desc"><?php echo wp_kses( $description, $valid_tags ); ?></p>
			</div>
		</div>
		<?php
		...
		<div class="tag_media-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				<p><input id="<?php echo esc_attr($input_prefix . $option['id']); ?>-helper" class="not-to-collect newtag" size="16" value="" type="text">
					<input class="not-to-collect button tagadd" value="<?php esc_attr_e( 'Add', 'a13-onelander' ); ?>" type="button"></p>
	...
	elseif ( $option['type'] == 'slider' ) {
		...
		<div class="slider-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				<?php apollo13framework_input_help_tip( __( 'Use slider to set proper value. You can click on slider handle and then use arrows keys(on keyboard) to adjust value precisely. You can also type in input value that is in/out of range of slider, and it will be used.', 'a13-onelander' ) ); ?>
		...
		<div class="select-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				if ( $products ) {
					...
					echo '<option value="0">'.esc_html__( 'None', 'a13-onelander' ).'</option>';
					...
					echo '<span class="empty-type">'. esc_html__( 'There are no products yet!', 'a13-onelander' ) . '</span>';
				}
	...
	elseif ( $option['type'] === 'proofing_items' ) {
		if(function_exists('a13fe_album_proofing_return_approved_files')){
			...
			if(sizeof($approved_array) === 0){
				...
				$value = '**'.__('Adobe Lightroom', 'a13-onelander').'**';

				$value .= "\r\n".implode(', ', $approved_array);

				$value .= "\r\n\r\n**".__('Windows Explorer or Mac Finder', 'a13-onelander').'**';
