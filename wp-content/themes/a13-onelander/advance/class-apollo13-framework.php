...
class Apollo13Framework
{
    ...
    function start()
    {
         ...
         */
        define('A13FRAMEWORK_TPL_SLUG', 'a13-onelander');//it is not always same as directory of theme
        define('A13FRAMEWORK_OPTIONS_NAME_PART', 'A13 OneLander');
        define('A13FRAMEWORK_THEME_ID_NUMBER', '20953926');
        define('A13FRAMEWORK_OPTIONS_NAME', 'apollo13_option_onelander');
        define('A13FRAMEWORK_CACHE', 'apollo13_onelander_cache');
...
//		define('A13FRAMEWORK_CUSTOM_POST_TYPE_WORK_SLUG', 'work'); //just to show it also exist - defined in real a bit lower
    ...
    function setup()
    {
        ...
        //LANGUAGE
        load_theme_textdomain( 'a13-onelander', get_theme_file_path( 'languages' ) );
        ...
        register_nav_menus(array(
            'header-menu' => __( 'Site Navigation', 'a13-onelander' ),
            'top-bar-menu' => __( 'Alternative short top bar menu', 'a13-onelander' ),
        ));
    }
...
//            'images_manager' //no parent options here
    ...
    function is_companion_plugin_ready($fail_message = false, $silent = false){
        ...
        if ( empty( $plugins[$plugin_file] ) ) {
            ...
            $label = __( 'Install', 'a13-onelander' ) . ' : ' . __( 'Apollo13 Framework Extensions', 'a13-onelander' );
        }
        ...
        elseif ( is_plugin_inactive( $plugin_path ) ){
            ...
            $label = __( 'Activate', 'a13-onelander' ) . ' : ' . __( 'Apollo13 Framework Extensions', 'a13-onelander' );
        }
        ...
        elseif( version_compare( $plugins[$plugin_file]['Version'], A13FRAMEWORK_MIN_COMPANION_VERSION, '<'  ) ){
            ...
            $label = __( 'Update', 'a13-onelander' ) . ' : ' . __( 'Apollo13 Framework Extensions', 'a13-onelander' );
        }

        if(!$ready && !$silent){
            $message = $fail_message ? $fail_message : __( 'This feature requires Apollo13 Framework Extensions plugin to be active and in the proper version.', 'a13-onelander' );
