    ...
    function apollo13framework_cpt_meta_fields(){
        ...
        for($i = 0; $i <= $custom_fields_number; $i++){
	        if( $i === 0 ) {
		        ...
		        if ( strlen( $temp ) ) {
			        $temp = esc_html__( 'Website', 'a13-onelander' ).':'.$temp;
		        }
	        }
	        else{
		        //custom fields
...
if(!function_exists('apollo13framework_loop_pagination')){
	...
	function apollo13framework_loop_pagination($args){
		$args['type'] = 'plain';
		$args['prev_text'] = _x( 'Previous', 'previous product', 'a13-onelander' );
		$args['next_text'] = _x( 'Next', 'next product', 'a13-onelander' );
...
function apollo13framework_print_media_filters($id = null){
	...
	if ( count( $filters ) ){
		...
		echo '<li class="selected" data-filter="__all"><a href="' . esc_url( apollo13framework_current_url() ) . '">' . esc_html__( 'All', 'a13-onelander' ) . '</a></li>';
...
function apollo13framework_print_proofing_filters($id = null){
	...
	<ul class="category-filter single-album-proofing-filter clearfix">
		<li class="selected" data-filter="__all"><a><?php esc_html_e( 'All', 'a13-onelander' ); ?></a></li>
		<li data-filter="1"><a><?php esc_html_e( 'Selected', 'a13-onelander' ); ?><i class="count"></i></a></li>
		<li data-filter="0"><a><?php esc_html_e( 'Not selected', 'a13-onelander' ); ?><i class="count"></i></a></li>
		<li data-filter="2"><a><?php esc_html_e( 'Commented', 'a13-onelander' ); ?><i class="count"></i></a></li>
	</ul>
	<div id="done-with-proofing" <?php echo $hide_button? ' class="idle"' : ''; ?>>
		<div class="not-done" title="<?php esc_attr_e( 'When you are done with commenting and selecting photos/videos, click this button.', 'a13-onelander' ); ?>"><?php esc_html_e( 'I am done with Selecting Items', 'a13-onelander' ); ?></div>
		<div class="loading"><?php esc_html_e( 'Working ...', 'a13-onelander' ); ?></div>
		<div class="done"><?php esc_html_e( 'Accepted Items:', 'a13-onelander' ); ?> <span class="counter"></span> <i class="fa fa-check"></i></div>
	</div>
	<?php
}
