...
if ( ! function_exists( 'apollo13framework_get_header_toolbar' ) ) {
	...
	function apollo13framework_get_header_toolbar( &$icons ) {
		...
		//prepare icons HTML
		$tools_html =
		        ( $basket_sidebar ? '<button id="basket-menu-switch" class="'.esc_attr($basket_sidebar_icon).' tool" title="' . esc_attr__( 'Shop sidebar', 'a13-onelander' ) . '"><span id="basket-items-count" class="zero">' . esc_html( $woocommerce->cart->cart_contents_count ) . '</span><span class="screen-reader-text">' . esc_attr__( 'Shop sidebar', 'a13-onelander' ) . '</span></button>' : '' ) .
		        ( $header_search ? '<button id="search-button" class="'.esc_attr($header_search_icon).' tool" title="' . esc_attr_x( 'Search', 'action', 'a13-onelander' ) . '"><span class="screen-reader-text">' . esc_attr_x( 'Search', 'action', 'a13-onelander' ) . '</span></button>' : '' ) .
		        ( $hidden_sidebar ? '<button id="side-menu-switch" class="'.esc_attr($hidden_sidebar_icon).' tool" title="' . esc_attr__( 'More info', 'a13-onelander' ) . '">'.($hs_type === 'animated'? '<i></i>' : '').'<span class="screen-reader-text">' . esc_attr__( 'More info', 'a13-onelander' ) . '</span></button>' : '' ) .
		        ( $menu_overlay ? '<button id="menu-overlay-switch" class="'.esc_attr($menu_overlay_icon).' tool" title="' . esc_attr__( 'Main menu', 'a13-onelander' ) . '">'.($mo_type === 'animated'? '<i></i>' : '').'<span class="screen-reader-text">' . esc_attr__( 'Main menu', 'a13-onelander' ) . '</span></button>' : '' ) .
		        ( $allow_mobile_menu ? '<button id="mobile-menu-opener" class="'.esc_attr($mobile_menu_icon).' tool" title="' . esc_attr__( 'Main menu', 'a13-onelander' ) . '">'.($mm_type === 'animated'? '<i></i>' : '').'<span class="screen-reader-text">' . esc_attr__( 'Main menu', 'a13-onelander' ) . '</span></button>' : '' ) .
