...
if(!function_exists('apollo13framework_search_form')){
	...
	function apollo13framework_search_form(
        ...
        $field_search = '<input' .
            ' placeholder="' . esc_attr__('Search &hellip;', 'a13-onelander' ) . '" ' .
        ...
        $form = '
                <form class="search-form" role="search" method="get" action="' . esc_url( home_url( '/' ) ) . '" >
                    <fieldset class="semantic">
                        ' . $field_search . '
                        <input type="submit" id="searchsubmit' . esc_attr( $search_id ) . '" title="'. esc_attr_x( 'Search', 'action', 'a13-onelander' ) .'" value="'. esc_attr_x( 'Search', 'action', 'a13-onelander' ) .'" />
		...
		if(strlen($title) === 0){
			...
			elseif ( $home ){
				if(get_option('page_for_posts') === '0'){
					$title =  esc_html__( 'Blog', 'a13-onelander' );
				}
				else{
			        ...
			        <div id="post-<?php the_ID(); ?>" <?php post_class('content-box'); ?>>
				        <div class="formatter">
				            ...
				            <div class="real-content">
				                ...
				                <?php
				                wp_link_pages( array(
				                        'before' => '<div id="page-links">'.esc_html__( 'Pages: ', 'a13-onelander' ),
				                        'after'  => '</div>')
				                );
				                ?>
				            </div>
	...
	function apollo13framework_make_post_grid_filter( $terms, $filter_class = '', $selected_term = '__all', $show_all = true ) {
		if( count( $terms ) ){
			...
			if( $selected_term === '__all' || $show_all ){
				echo '<li '.( $selected_term === '__all' ? ' class="selected"' : '' ).' data-filter="__all"><a href="' . esc_url( apollo13framework_current_url() ) . '">' . esc_html__( 'All', 'a13-onelander' ) . '</a></li>';
			}
