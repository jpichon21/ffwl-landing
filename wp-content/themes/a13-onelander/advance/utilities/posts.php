...
if(!function_exists( 'apollo13framework_get_post_meta_data' )){
    ...
    function apollo13framework_get_post_meta_data() {
        ...
        if(
            ...
            $return .= esc_html__( 'Updated: ', 'a13-onelander' ).apollo13framework_modified_on();
        }
...
if(!function_exists('apollo13framework_posted_by_author')){
	...
	function apollo13framework_posted_by_author() {
        ...
        return
			...
			sprintf(  esc_html__( 'by %s ', 'a13-onelander' ),
                sprintf('<a class="vcard author" href="%1$s" title="%2$s" %4$s><span class="fn" %5$s>%3$s</span></a>',
				    esc_url(get_author_posts_url( get_the_author_meta( 'ID' ) )),
                    /* translators: %s - author name */
				    sprintf( esc_attr__( 'View all posts by %s', 'a13-onelander' ), get_the_author() ),
				    get_the_author(),
...
if(!function_exists('apollo13framework_under_post_content')){
    function apollo13framework_under_post_content() {
        ...
        wp_link_pages( array(
                'before' => '<div id="page-links"><span class="page-links-title">'. esc_html__( 'Pages: ', 'a13-onelander' ).'</span>',
                'after'  => '</div>'
	        )
        );
            ...
            if($is_prev || $is_next){
                ...
                if($is_prev){
                    ...
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item prev">'
                         .'<span><i class="fa fa-long-arrow-'.( is_rtl() ? 'right' : 'left' ).'"></i> '.esc_html__( 'Previous article', 'a13-onelander' ).'</span>'
                ...
                if($is_next){
                    ...
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item next">'
                         .'<span>'.esc_html__( 'Next article', 'a13-onelander' ).' <i class="fa fa-long-arrow-'.( is_rtl() ? 'left' : 'right' ).'"></i></span>'
...
if(!function_exists('apollo13framework_new_excerpt_more')){
    ...
    function apollo13framework_new_excerpt_more($link) {
        ...
        //return read more
        return '&hellip;
 <p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'a13-onelander' ).'</a></p>';
    }
}
...
if(!function_exists('apollo13framework_has_excerpt_read_more')){
    ...
    function apollo13framework_has_excerpt_read_more($content) {
        ...
        if( $apollo13framework_a13->get_option( 'blog_read_more', 'on' ) === 'on' && has_excerpt() ){
            $content .= '<p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'a13-onelander' ).'</a></p>';
        }
    ...
    function apollo13framework_comments_navigation() {
        if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
            ...
            <nav class="navigation comment-navigation">
                <h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'a13-onelander' ); ?></h2>
                <div class="nav-links">
                    <?php
                    if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'a13-onelander' ) ) ) :
                        printf( '<div class="nav-previous">%s</div>', wp_kses_post( $prev_link ) );
                    endif;

                    if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'a13-onelander' ) ) ) :
        ...
        if ( ! $query->have_posts() ):
            ...
            <div class="formatter">
                <div class="real-content empty-blog">
                    <?php
                    if( !is_search() ){
                        echo '<p>'.esc_html__( 'Apologies, but no results were found for the requested archive.', 'a13-onelander' ).'</p>';
                    }
                    ...
                    if(post_password_required()){
                        ...
                        <div class="formatter">
                            ...
                            <div class="real-content">
                                <p><?php esc_html_e( 'To view it please enter your password below', 'a13-onelander' ); ?></p>
