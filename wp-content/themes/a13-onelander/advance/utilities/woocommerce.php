	...
	function apollo13framework_wc_get_term_parents( $id, $taxonomy, $link = false, $separator = '/', $nice_name = false, $visited = array() ) {
		...
		if ( $link ) {
			$chain .= '<a href="' . esc_url( get_term_link( $parent, $taxonomy ) ) . '" title="' .
			          ...
			          esc_attr( sprintf( esc_html__( "View all posts in %s", 'a13-onelander' ), $parent->name ) )
...
if(!function_exists( 'apollo13framework_wc_single_product_labels' )){
	...
	function apollo13framework_wc_single_product_labels() {
        ...
        if(!$product->is_in_stock()){
            $html .= '<span class="ribbon out-of-stock"><em>'.esc_html__( 'Out of stock', 'a13-onelander' ).'</em></span>';
        }
        else{
            //sale
            if($product->is_on_sale()){
                $html .= '<span class="ribbon sale"><em>'.esc_html__( 'Sale', 'a13-onelander' ).'</em></span>';
            }
            //new
            if(apollo13framework_is_product_new()){
                $html .= '<span class="ribbon new"><em>'.esc_html__( 'New', 'a13-onelander' ).'</em></span>';
            }
        }
...
if( ! function_exists( 'apollo13framework_wc_min_cart_footer' ) ){
	...
	function apollo13framework_wc_min_cart_footer() {
		if( WC()->cart->is_empty() ):
			?>
			<p class="buttons">
				<a href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ); ?>" class="button wc-forward"><?php esc_html_e( 'Go to shop', 'a13-onelander' ); ?></a>
			</p>
			<?php
		endif;
	}
