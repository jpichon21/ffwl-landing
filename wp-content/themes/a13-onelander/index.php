...
//Lets decide what is the title
	if ( is_search() ) {
		...
		$_title = sprintf( esc_html( _n( '%1$d search result for "%2$s"', '%1$d search results for "%2$s"', $count, 'a13-onelander' ) ), $count, get_search_query() );
	} elseif ( is_archive() ) {
		if ( is_author() ) {
			/* translators: %s - author name */
			$_title = sprintf( esc_html__( 'Author Archives: %s', 'a13-onelander' ), "<span class='vcard'>" . get_the_author() . "</span>" );
		} elseif ( is_category() ) {
			/* translators: %s - category name */
			$_title = sprintf( esc_html__( 'Category Archives: %s', 'a13-onelander' ), '<span>' . single_cat_title( '', false ) . '</span>' );
		} elseif ( is_tag() ) {
			/* translators: %s - tag name */
			$_title = sprintf( esc_html__( 'Tag Archives: %s', 'a13-onelander' ), '<span>' . single_tag_title( '', false ) . '</span>' );
		} elseif ( is_day() ) {
			/* translators: %s - day */
			$_title = sprintf( esc_html__( 'Daily Archives: %s', 'a13-onelander' ), '<span>' . get_the_date() . '</span>' );
		} elseif ( is_month() ) {
			/* translators: %s - month */
			$_title = sprintf( esc_html__( 'Monthly Archives: %s', 'a13-onelander' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );
		} elseif ( is_year() ) {
			/* translators: %s - year */
			$_title = sprintf( esc_html__( 'Yearly Archives: %s', 'a13-onelander' ), '<span>' . get_the_date( 'Y' ) . '</span>' );
		} else {
			$_title = esc_html__( 'Blog Archives', 'a13-onelander' );
		}
	}
