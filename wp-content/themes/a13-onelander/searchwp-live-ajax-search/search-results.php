...
<!--		--><?php //$post_type = get_post_type_object( get_post_type() ); echo esc_html( $post_type->labels->singular_name ); ?>
	...
	?>
<div><a href="<?php echo esc_url( apollo13framework_current_url().'?s='.$_search ); ?>" class="all-results"><?php esc_html_e('View all results', 'a13-onelander'); ?></a></div>
<?php else : ?>
	<div class="searchwp-live-search-no-results">
		<h2><?php esc_html_e( 'No results found.', 'a13-onelander' ); ?></h2>
		<em><?php esc_html_e( 'Please try another search term', 'a13-onelander' ); ?></em>
	</div>
<?php endif; ?>
